...
type (
	...
	ClientTx interface {
		...
		SetMsgs(...original.Msg) error
		GetSignatures() []original.Signature
...
func (f Factory) BuildAndSignedTx(msgs ...original.Msg) ([]byte, error) {
...
func (f Factory) BuildUnsignedTx(msgs ...original.Msg) (ClientTx, error) {
	if f.chainID == "" {
	...
	if !f.gasPrices.IsZero() {
		...
		glDec := original.NewDec(int64(f.gas))

		// Derive the fees based on the provided gas prices, where
		// fee = ceil(gasPrice * gasLimit).
		fees = make(original.Coins, len(f.gasPrices))

		for i, gp := range f.gasPrices {
			fee := gp.Amount.Mul(glDec)
			fees[i] = original.NewCoin(gp.Denom, fee.Ceil().RoundInt())
		}
	}
...
// built.
func (f Factory) BuildSimTx(msgs ...original.Msg) ([]byte, error) {
	tx, err := f.BuildUnsignedTx(msgs...)
	if err != nil {
		return nil, err
	}
