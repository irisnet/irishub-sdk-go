...
func (s serviceClient) queryResponseByTxQuery(requestID string) (response, error) {
	builder := original.NewEventQueryBuilder().
		AddCondition(original.NewCond(original.EventTypeMessage,
			original.AttributeKeyAction).EQ("respond_service")).
		AddCondition(original.NewCond(original.EventTypeMessage,
			original.AttributeKeyAction).EQ(attributeKeyRequestID))
