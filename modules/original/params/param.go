...
func (p paramClient) RegisterCodec(cdc original.Codec) {
...
func Create(ac original.BaseClient) rpc.Params {
	return paramClient{
		BaseClient: ac,
		Logger:     ac.Logger(),
	}
...
func (p paramClient) QueryParams(moduleName string) (rpc.ParamsResponses, original.Error) {
	var prs paramsResponses
	if len(moduleName) == 0 {
		for _, v := range AllModule {
			res, err := p.queryParams(v)
			if err != nil {
				return rpc.ParamsResponses{}, original.Wrap(err)
			}
			prs = append(prs, res)
		}
	} else {
...
func (p paramClient) queryParams(moduleName string) (paramsResponse, original.Error) {
	routers := getRouters(moduleName)
	if len(routers) == 0 {
		return paramsResponse{}, original.Wrapf("the params of module isn't exist")
	}
	...
	for _, router := range routers {
		...
		if err != nil {
			return paramsResponse{}, original.Wrap(err)
		}
