...
func validateInputSchema(inputSchema map[string]interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal the input schema: %s", err)
	}

	_, err = gojsonschema.NewSchema(gojsonschema.NewBytesLoader(inputSchemaBz))
	if err != nil {
		return fmt.Errorf("invalid input schema: %s", err)
	}
...
func validateOutputSchema(outputSchema map[string]interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal the output schema: %s", err)
	}

	_, err = gojsonschema.NewSchema(gojsonschema.NewBytesLoader(outputSchemaBz))
	if err != nil {
		return fmt.Errorf("invalid output schema: %s", err)
	}
...
func parseServiceSchemas(schemas string) (ServiceSchemas, error) {
	...
	if err := json.Unmarshal([]byte(schemas), &svcSchemas); err != nil {
		return svcSchemas, fmt.Errorf("failed to unmarshal the schemas: %s", err)
	}
...
func parseInputSchema(schemas string) ([]byte, error) {
	...
	if err2 != nil {
		return nil, fmt.Errorf("failed to marshal the input schema: %s", err2)
	}
...
func parseOutputSchema(schemas string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal the output schema: %s", err2)
	}
